...
export function TicketCard({ 
  ...
  useEffect(() => {
    const updateSlaInfo = () => {
      ...
      const timeDifference = limitTime.getTime() - now.getTime();
      ...
      // Set status based on the configurable risk threshold:
      // - Cards within configured risk threshold days are yellow (at risk)
      // - Expired cards are red
      // - All others are green (within deadline)
      
      if (now > limitTime) {
        // Already expired
        setSlaStatus("red");
      } else if (daysDifference >= 0 && daysDifference <= riskThresholdDays && !isWeekend) {
        // Within risk threshold and not a weekend
        setSlaStatus("yellow");
      } else {
        // All other cases (future expiration beyond risk threshold)
        setSlaStatus("green");
      }
  ...
  }, [data_abertura, sla_limite, status, riskThresholdDays]);
