...
export function TicketDashboard() {
  ...
  const sortedTickets = [...tickets].sort((a, b) => {
    ...
    const getSlaStatus = (ticket: Ticket) => {
      ...
      if (ticket.status === "Em andamento") {
        ...
        // Check if expiration is today (same day)
        const today = new Date();
        const isToday = 
          slaLimit.getDate() === today.getDate() && 
          slaLimit.getMonth() === today.getMonth() && 
          slaLimit.getFullYear() === today.getFullYear();
        
        if (isToday && today <= slaLimit) {
          return 1; // EM RISCO - highest priority (prestes a expirar no dia)
        } else if (today > slaLimit) {
          return 2; // EXPIRADO - second highest priority
        } else {
          return 3; // DENTRO DO PRAZO - third priority
        }
  ...
  // Helper function to check if a ticket is "Em Risco" (prestes a expirar no dia)
  const isTicketEmRisco = (ticket: Ticket) => {
    ...
    // Check if the SLA expiration date is today
    const isToday = 
      slaLimit.getDate() === now.getDate() && 
      slaLimit.getMonth() === now.getMonth() && 
      slaLimit.getFullYear() === now.getFullYear();
    ...
    // A ticket is at risk only if:
    // 1. Expiration is today
    // 2. It's not a weekend day
    // 3. It hasn't expired yet
    return isToday && !isWeekend && now <= slaLimit;
  };
  ...
  const isTicketSaudavel = (ticket: Ticket) => {
    ...
    if (ticket.status === "Em andamento") {
      return !isTicketEmRisco(ticket) && !isTicketEstourado(ticket);
    }
  ...
  const getFilteredTickets = () => {
    ...
    switch (activeTab) {
      ...
      case "estourados":
        ...
        break;
      case "andamento":
        result = teamProjectFiltered.filter(ticket => ticket.status === "Em andamento");
        break;
      case "pausados":
  ...
  const calculateSummaryStats = () => {
    ...
    baseFilteredTickets.forEach((ticket: Ticket) => {
      if (ticket.status === 'Pausado' || ticket.status === 'Resolved') {
        ...
        // Categorize based on SLA status
        if (isTicketEstourado(ticket)) {
          estourados += 1;
        } else if (isTicketEmRisco(ticket)) {
          emRisco += 1;
...
{/* BI Integration buttons removed as requested */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div className="flex items-center h-full px-4">
            ...
            <div className="flex-grow">
              <div className="flex justify-between items-center">
                ...
                <span className="text-4xl font-bold text-green-500">{summaryStats.saudaveis}</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card className={`border-l-8 border-l-blue-500 h-24 ${config.darkMode ? 'bg-gray-800 border-gray-700' : ''}`}>
          <div className="flex items-center h-full px-4">
            <div className="flex-grow">
              <div className="flex justify-between items-center">
                <span className={`font-bold text-xl ${config.darkMode ? 'text-white' : ''}`}>TOTAL</span>
                <span className={`text-4xl font-bold ${config.darkMode ? 'text-white' : ''}`}>{summaryStats.total}</span>
              </div>
            </div>
          </div>
        </Card>
      ...
      >
        <TabsList className={`grid grid-cols-5 h-16 ${config.darkMode ? 'bg-gray-800' : ''}`}>
          ...
          <TabsTrigger 
            ...
            className={`text-2xl font-bold ${
              config.darkMode 
                ? activeTab === "andamento" ? "bg-green-900/40 text-green-300" : "text-gray-300"
                : activeTab === "andamento" ? "bg-green-100" : ""
            }`}
          >
            EM ANDAMENTO ({summaryStats.saudaveis + summaryStats.emRisco + summaryStats.estourados})
          </TabsTrigger>
          <TabsTrigger 
            value="pausados" 
            className={`text-2xl font-bold ${
        ...
        {["emRisco", "estourados", "andamento", "pausados", "all"].map(tab => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
              {loading ? (
                ...
                getFilteredTickets().map((ticket) => (
                  <TicketCard key={ticket.id_chamado} {...ticket} />
                ))
              )}
            </div>
